...
from .graph_post import merge_close_junctions
...
class RunParams:
    ...
    # Junction merge (NEW)
    junction_merge_radius_px: int = 0  # 0 disables; try 2..10
...
def run_pipeline(image_path: Path, p: RunParams) -> Dict[str, Any]:
    ...
    merge_stats = merge_close_junctions(g, radius_px=int(p.junction_merge_radius_px))
    ...
    lengths_px = np.array(
        [float(f.length_px) for f in fibers_final if f.length_px is not None and f.length_px > 0],
        dtype=np.float64,
    )
